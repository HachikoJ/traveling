...
{t:'4.30花山→靖西高速堵车',a:'导航切国道，或服务区休息等1h再走'},
{t:'5.2深圳北换乘来不及',a:'改签下一班，不要跑！安全第一'},
{t:'5.3赶海潮汐不配合',a:'海边散步看日落，一样开心'},
{t:'5.4返程堵车',a:'提前出发，走国道备选，实时看导航路况'},
...
function renderOverview(){
  ...
  let h=`<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
    <div class="bg-white rounded-xl p-4 shadow-sm text-center"><div class="text-2xl font-bold text-teal-600">8</div><div class="text-xs text-gray-500">总天数</div></div>
    <div class="bg-white rounded-xl p-4 shadow-sm text-center"><div class="text-2xl font-bold text-blue-600">4</div><div class="text-xs text-gray-500">高铁程</div></div>
    <div class="bg-white rounded-xl p-4 shadow-sm text-center"><div class="text-2xl font-bold text-emerald-600">7</div><div class="text-xs text-gray-500">景区</div></div>
    <div class="bg-white rounded-xl p-4 shadow-sm text-center"><div class="text-2xl font-bold text-purple-600">6</div><div class="text-xs text-gray-500">换酒店</div></div>
  </div>`;
  h+=`<div class="bg-white rounded-xl shadow-sm p-4 mb-6"><h2 class="text-base font-bold text-gray-800 mb-3 flex items-center gap-2">📊 时间线一览 <span class="text-xs text-gray-400 font-normal">← 横向滚动</span></h2><div class="gw"><div class="gi">`;
  h+='<div class="flex mb-2 text-xs text-gray-400" style="position:relative;height:16px">';
  for(let hr=GS;hr<=GE;hr+=3)h+=`<span style="position:absolute;left:${gp(hr*60)}%;transform:translateX(-50%)">${hr}:00</span>`;
  h+='</div>';
  DI.forEach(di=>{
    const sg=IT.filter(s=>s.d===di.d),dn=sg.filter(s=>S.done[s.id]).length;
    const pct=sg.length?Math.round(dn/sg.length*100):0;
    h+=`<div class="flex items-center gap-2 mb-1.5"><div class="w-14 text-xs font-medium text-gray-600 flex-shrink-0">${di.dt}<br><span class="text-gray-400">${di.wk}</span></div><div class="gr flex-1">`;
    ...
    sg.forEach(seg=>{
      ...
      h+=`<div class="gs ${op}" style="left:${sp}%;width:${w}%;background:${TC[seg.tp]}" title="${seg.t}(${fm(seg.s)}-${fm(seg.e)})">${seg.t.substring(0,6)}</div>`;
    });
  ...
  Object.entries(TC).forEach(([k,v])=>{
    const lb={train:'高铁',drive:'自驾',attraction:'景区',meal:'餐饮',hotel:'酒店',rest:'休息',other:'其他'}[k]||k;
    h+=`<span class="flex items-center gap-1 text-xs text-gray-600"><span class="w-3 h-3 rounded-sm" style="background:${v}"></span>${lb}</span>`;
  });
  h+='</div></div>';
  const places=['出发','南宁东','崇左南','大新','靖西','南宁','深圳北','仲恺','惠州','汕尾','深圳'];
  const cls=['bg-blue-50 text-blue-700','bg-teal-50 text-teal-700','bg-teal-50 text-teal-700','bg-emerald-50 text-emerald-700','bg-emerald-50 text-emerald-700','bg-teal-50 text-teal-700','bg-blue-50 text-blue-700','bg-teal-50 text-teal-700','bg-emerald-50 text-emerald-700','bg-emerald-50 text-emerald-700','bg-blue-50 text-blue-700'];
  h+=`<div class="bg-white rounded-xl shadow-sm p-4 mb-6"><h2 class="text-base font-bold text-gray-800 mb-3">🗺️ 行程路线</h2><div class="flex flex-wrap items-center gap-1.5 text-sm">`;
  places.forEach((p,i)=>{if(i>0)h+='<span class="text-gray-300">→</span>';h+=`<span class="${cls[i]} px-2 py-1 rounded-full text-xs font-medium">${p}</span>`});
  h+='</div></div>';
  ...
  DI.forEach(di=>{
    ...
    h+=`<div class="bg-white rounded-xl shadow-sm p-4 cursor-pointer hover:shadow-md transition" onclick="swTab('daily');scrollToDay(${di.d})">
...
function renderDaily(){
  ...
  DI.forEach(di=>{
    ...
    const defaultOpen=isToday||(!todayD&&di.d===1);
    h+=`<div id="day-${di.d}" class="bg-white rounded-xl shadow-sm overflow-hidden mb-6 ${isToday?'ring-2 ring-amber-400 ring-offset-1':''}">
      ...
      </div><div class="p-3 sm:p-4 transition-all duration-300 ${defaultOpen?'':'hidden'}" id="day-body-${di.d}">`;
    sg.forEach(seg=>{
      ...
      const fixedTag=seg.fx?'<span class="ml-1 text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded">🔒固定</span>':'';
      const actTime=S.act[seg.id]?`<span class="text-xs ${devCls} ml-1">实际${fm(S.act[seg.id])}(${devStr})</span>`:'';
      const warnHtml=seg.warn?`<div class="mt-1.5 bg-amber-50 border border-amber-200 rounded-lg p-2 text-xs text-amber-700 font-medium">⚠️ ${seg.warn}</div>`:'';
      h+=`<div class="sc ${bgc} ${done?'done':''}" id="card-${seg.id}">
        <div class="flex items-start gap-3">
          <div class="ck ${done?'on':''}" onclick="toggleSeg('${seg.id}')">${done?'✓':''}</div>
          <div class="flex-1 min-w-0">
            <div class="flex items-center flex-wrap gap-1"><span class="text-sm">${seg.ic}</span><span class="st font-medium text-gray-800 text-sm">${seg.t}</span>${fixedTag}${adjStr}</div>
...
function renderGuide(){
  ...
  el.innerHTML=`<div class="space-y-4">
    <div class="bg-white rounded-xl shadow-sm p-5"><h2 class="text-base font-bold text-gray-800 mb-4">👶 带两岁宝宝出行指南</h2><div class="space-y-3">
      ...
      <div class="border-l-[3px] border-green-400 bg-green-50 rounded-r-lg p-3"><h3 class="font-medium text-green-800 text-sm">安全第一</h3><p class="text-xs text-green-700 mt-1">德天瀑布边有护栏但需看护；鹅泉赶海玩水必须全程牵手；自驾务必用安全座椅。</p></div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-5"><h2 class="text-base font-bold text-gray-800 mb-4">🚗 自驾注意事项</h2><div class="space-y-3">
      ...
      <div class="border-l-[3px] border-cyan-400 bg-cyan-50 rounded-r-lg p-3"><h3 class="font-medium text-cyan-800 text-sm">还车</h3><p class="text-xs text-cyan-700 mt-1">提前联系门店确认营业时间。加满油、清理、拍照。预留30-40min手续。</p></div>
    </div></div>
    <div class="bg-white rounded-xl shadow-sm p-5"><h2 class="text-base font-bold text-gray-800 mb-4">🏞️ 景区贴士</h2><div class="space-y-3">
      <div class="border border-gray-100 rounded-lg p-3"><h3 class="font-bold text-sm">🌊 德天跨国瀑布</h3><p class="text-xs text-gray-600 mt-1">📍崇左大新·55km | 🎫80元+观光车35元+竹筏30元 | ⏰4-5h | 💡旺季9点前入园 | 👶推车可行有台阶建议背带</p></div>
      <div class="border border-gray-100 rounded-lg p-3"><h3 class="font-bold text-sm">🏞️ 明仕田园</h3><p class="text-xs text-gray-600 mt-1">📍崇左大新·17km | 🎫80元+竹筏120元 | ⏰2-3h | 💡清晨人少景美 | 👶竹筏宝宝友好穿救生衣</p></div>
      <div class="border border-gray-100 rounded-lg p-3"><h3 class="font-bold text-sm">🏔️ 宁明花山岩画</h3><p class="text-xs text-gray-600 mt-1">📍崇左宁明·世遗 | 🎫80元含船票 | ⏰2-3h | 💡乘船观看船程30min | 👶船平稳宝宝友好</p></div>
      <div class="border border-gray-100 rounded-lg p-3"><h3 class="font-bold text-sm">💧 鹅泉</h3><p class="text-xs text-gray-600 mt-1">📍百色靖西·西南三名泉 | 🎫48元 | ⏰2-3h | 💡清晨如镜倒影美 | 👶步道平缓推车可行</p></div>
      <div class="border border-gray-100 rounded-lg p-3"><h3 class="font-bold text-sm">🌸 千花洲</h3><p class="text-xs text-gray-600 mt-1">📍惠州惠城 | 🎫60元 | ⏰2-3h | 💡五月花海有儿童区 | 👶亲子友好步道平坦</p></div>
      <div class="border border-gray-100 rounded-lg p-3"><h3 class="font-bold text-sm">🦀 汕尾赶海</h3><p class="text-xs text-gray-600 mt-1">📍红海湾/品清湖 | 🎫免费 | ⏰退潮前后2h | 💡⚠️查潮汐表！带桶铲穿防滑鞋 | 👶全程牵手注意深坑水母</p></div>
      <div class="border border-gray-100 rounded-lg p-3"><h3 class="font-bold text-sm">🌀 风车岛</h3><p class="text-xs text-gray-600 mt-1">📍汕尾遮浪半岛 | 🎫免费 | ⏰3-4h | 💡风车+海景出片可环岛自驾 | 👶海边风大保暖</p></div>
    </div></div>
...
// ===== 行李清单 =====
const PACK=[
  {cat:'📄 证件',items:['身份证(4人)','宝宝出生证明/户口本','高铁票/订单截图','驾照','租车订单','酒店预订确认']},
